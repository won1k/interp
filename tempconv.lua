...
function eval(test_data, model)
	...
	local accuracy = 0
	for i = 1, n do
		if maxidx[i] == test_data.output[i + torch.floor(opt.dwin/2)] then
			accuracy = accuracy + 1
		end
	end
	return accuracy/n
...
function test(test_data, model, criterion)
	local test_pred = model:forward(test_data.input)
	local test_output = test_data.output[{{torch.floor(opt.dwin/2) + 1, torch.floor(opt.dwin/2) + test_pred:size(1)}}]
	local maxval, maxidx = test_pred:max(2)
	maxidx = maxidx:squeeze()
